package dev.lupluv.kc.utils;

import dev.lupluv.kc.Cat;
...
public class Util {
    ...
    public MessageEmbed getInfoEmbed(User user, Member member, Guild guild){
        ...
        if(member.getRoles().contains(guild.getRoleById("909203442415853688"))) {
            eb.addField("Muted?", "Yes", true);
        }else if(!member.getRoles().contains(guild.getRoleById("909203442415853688"))) {
    ...
    public void updateMutedRolePermissions(){
        Guild guild = Cat.jda.getGuildById("821823858734530571");
    ...
    public void sendTicketsCreationMessage(Guild guild, TextChannel tc){
        ...
        eb.setFooter("kirstenjbx's shed - Ticket Support", Cat.jda.getSelfUser().getAvatarUrl());
    ...
    public void loadTranscript(ButtonInteractionEvent event) throws IOException {
        ...
        File transcript = new File("/var/www/lupluv.dev/transcripts/data/" + uid + ".html");
        File template = new File("/var/www/lupluv.dev/transcripts/template.html");
        if(!transcript.exists()){
            ...
            event.getChannel().sendMessageEmbeds(eb.build()).addActionRow(
                    Button.link("https://lupluv.dev/transcripts/data/" + uid + ".html", "Open transcript")
            ).queue();
        }
    }
